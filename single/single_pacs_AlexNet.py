...
if __name__ == '__main__':
    ...
    parse.add_argument('--save_path', type=str, default='../results/pacs')
    parse.add_argument('--data', type=str, default='art_painting', help='[art_painting | cartoon | photo | sketch]')
    args = parse.parse_args()

    exp_floder = 'single_pacs_AlexNet'
    ...
    if log:
        log_path = os.path.join('../logs/pacs/', exp_floder)
    ...
    data_base_path = '../data'
    min_data_len = 5e8
    for site in ['art_painting', 'cartoon', 'photo', 'sketch']:
        trainset = PACSDataset(data_base_path, site, transformer=transform_train)
    ...
    trainset = PACSDataset(data_base_path, args.data, transformer=transform_train)
    testset = PACSDataset(data_base_path, args.data, transformer=transform_test, train=False)
    ...
    model = AlexNet.AlexNet(num_class=7).to(device)
    ...
    for epoch in range(start_epoch, start_epoch+N_EPOCHS):
        ...
        if val_acc > best_acc:
            ...
            if not os.path.exists('../results/pacs/' + args.data):
               os.mkdir('../results/pacs/'+ args.data)
            torch.save({
                'model': model.state_dict(),
                'best_epoch': best_epoch,
                'best_acc': best_acc,
                'epoch': epoch
            }, '../results/pacs/'+ args.data +'/{}.ckpt'.format(a))
            a = a + 1
